...
def _normalized_to_pixel_coordinates(
  ...
  # # Checks if the float value is between 0 and 1.  # 碰到邊邊會有異常框框
...
def M_face(image):
  with mp_face_detection.FaceDetection(min_detection_confidence=0.5, model_selection=1) as face_detection:
      ...
      image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  #image = cv2.cvtColor(cv2.flip(image, 1), cv2.COLOR_BGR2RGB)
      ...
      #cv2.imshow('MediaPipe Face Detection', image)  # 在副程式imshow會炸裂
